...
const ABSOLUTE_RULES = `## STEP 1 — 추출 전 반드시 수행
슬라이드 이미지를 보고 다음을 먼저 파악하세요:
A. 독립된 박스/카드/경계 구역이 몇 개인가? (상하좌우 전체 스캔)
B. 헤드라인·KPI·메인 텍스트가 있는가?
C. 표·파이프라인·비교 구조가 있는가?
→ 위에서 센 박스 수 = 최종 JSON의 카드/섹션 수. 더 적으면 누락이다.
...
5. ⛔ 우측·하단·우하단 박스 누락 금지 — N자 순서로 끝까지 읽을 것.`
