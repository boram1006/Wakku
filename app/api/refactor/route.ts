...
중요: 원본의 색상 테마(다크/라이트 무관)를 무시하고, 디자인 시스템의 라이트 테마로 완전히 교체하세요.
...
- 원본 HTML의 <style> 태그를 모두 삭제하세요. 다크 테마 CSS 변수, 커스텀 배경색, :root 변수 정의 등 일체를 제거합니다.
- body, html, *, :root에 배경색이나 색상을 임의로 지정하지 마세요. DS CSS가 처리합니다.
- 디자인 시스템으로 커버되지 않는 SVG 도형 색상, 이미지 크기처럼 정말 필요한 스타일만 최소한의 새 <style> 태그에 작성하세요.
