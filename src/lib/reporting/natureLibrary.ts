...
const NATURE_ENTRIES: NatureEntry[] = [
  ...
  // ── 4. agent-build ────────────────────────────────────────────────────────
  {
    ...
    canonicalPageRoles: ['hook', 'solution', 'execution', 'evidence', 'next-step'],
    typicalRequestedAction: 'support',
    commonEvidenceTypes: ['demo', 'screenshot', 'workflow', 'metric'],
    commonInteractionPatterns: ['step-reveal', 'demo-flow', 'detail-accordion'],
    whenToUse: [
      'AI 시스템/에이전트의 설계 구조를 처음 설명할 때',
      '구현 완료 전 중간 검토가 필요한 경우',
      '기술 구조 이해가 의사결정에 필요한 경우',
    ],
    whenNotToUse: [
