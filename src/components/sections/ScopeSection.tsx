            ...
            <ReportCard key={i} emphasis={card.emphasis}>
              {card.label && (
                <ReportLabel accent={card.labelAccent}>
                  <EditableText
                    target={{ type: 'card', sectionId: section.id, cardIdx: i, field: 'label', label: `카드 ${i + 1} 레이블`, multiline: false }}
                    as="span"
                  />
                </ReportLabel>
              )}
              <EditableText
                target={{ type: 'card', sectionId: section.id, cardIdx: i, field: 'title', label: `카드 ${i + 1} 제목`, multiline: false }}
                as="h3"
                className="report-card-title"
                style={{ marginTop: 14 }}
              />
              <EditableText
                target={{ type: 'card', sectionId: section.id, cardIdx: i, field: 'desc', label: `카드 ${i + 1} 본문`, multiline: true }}
                as="p"
