...
export function PagePatternExplorer({ onClose }: Props) {
  ...
  function handleSelect(patternId: string) {
    ...
    if (!pattern) return
    const newPage = pattern.createPage(pages)
    addPage(newPage)
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14 }}>
            {filtered.map((pattern) => (
              <PatternCard
                key={pattern.id}
                pattern={pattern}
                onSelect={() => handleSelect(pattern.id)}
              />
            ))}
          </div>
        </div>
...
function PatternCard({ pattern, onSelect }: { pattern: typeof PAGE_PATTERNS[0]; onSelect: () => void }) {
  ...
  return (
    <button
      ...
      style={{
        ...
        borderRadius: 14, background: hovered ? '#FFF8F8' : '#fff',
        cursor: 'pointer', padding: 0, overflow: 'hidden',
        transition: 'border-color .15s, background .15s, box-shadow .15s',
        boxShadow: hovered ? '0 4px 16px rgba(253,49,46,.12)' : '0 1px 3px rgba(0,0,0,.06)',
        textAlign: 'left',
      }}
    >
      {/* Mini preview area */}
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{
            ...
            background: hovered ? 'rgba(253,49,46,.1)' : '#E5E7EB',
            color: hovered ? '#FD312E' : '#6B7280',
